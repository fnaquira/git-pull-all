/*
URL original: https://github.com/mst5295/node-git-pull
*/
...
function gitPull(localAbsPath, branch) {
	shell.cd(localAbsPath);
	var retCode = shell.exec(
		'git pull ' + branch + ' --allow-unrelated-histories'
	).code;
	if (retCode !== 0) {
		shell.exit(1);
...
function gitRemote(localAbsPath, link) {
	shell.cd(localAbsPath);
	var retCode = shell.exec(
		'git remote add -f -t master -m master origin ' + link
	).code;
	if (retCode !== 0) {
		shell.exit(1);
